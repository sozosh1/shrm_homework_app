  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) async {
    try {
      ...
      if (response.data is String && _isJsonString(response.data)) {
        _talker.debug('🔄 Десериализация JSON через изолят: ${response.requestOptions.path}');
        ...
        if (deserializedData != null) {
          ...
          _talker.warning('⚠️ Не удалось десериализовать JSON через изолят, используем стандартную десериализацию');
        }
      }
    } catch (e, st) {
...
class TypedIsolateDeserializerInterceptor extends Interceptor {
  ...
  TypedIsolateDeserializerInterceptor(
    this._talker,
    this._typeFactories,
  );
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) async {
    try {
      ...
      if (factory != null && response.data is String && _isJsonString(response.data)) {
        ...
        if (deserializedData != null) {
          ...
          _talker.warning('⚠️ Не удалось десериализовать с типизацией, используем стандартную десериализацию');
        }
      }
    } catch (e, st) {
      _talker.error('❌ Ошибка при типизированной десериализации через изолят', e, st);
    }
